...
const viewPaths:Record<View,string>={home:"/app/",traders:"/app/traders/",community:"/app/community/",activity:"/app/activity/",positions:"/app/positions/",profile:"/app/profile/"};

function initialView():View{
  if(typeof window==="undefined") return "home";
  const section=location.pathname.replace(/^\/app\/?/,"").split("/")[0];
  if(routeViews[section]) return routeViews[section];
...
function positionMath(p:any){try{const original=BigInt(p.entryTokenRaw||"0"),remaining=BigInt(p.remainingTokenRaw||"0"),zero=BigInt(0),scale=BigInt(1_000_000);const fraction=original>zero?Number((remaining*scale)/original)/1_000_000:0;const remainingCost=Number(p.costUsd||0)*fraction;const currentValue=remainingCost+Number(p.unrealizedPnlUsd||0);const pnlPct=remainingCost>0?Number(p.unrealizedPnlUsd||0)/remainingCost*100:0;return{fraction,remainingCost,currentValue,pnlPct}}catch{return{fraction:0,remainingCost:0,currentValue:Number(p.unrealizedPnlUsd||0),pnlPct:0}}}
