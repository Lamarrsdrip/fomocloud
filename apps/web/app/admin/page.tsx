...
function AdminAlerts(){
 ...
 return <section className="app-card" style={{marginTop:12}}>
  ...
  {alerts===null?<div className="loading" style={{minHeight:80}}>Loading…</div>:alerts.length?<div className="list">{alerts.map(a=><div className="list-row" style={{gridTemplateColumns:"1fr auto"}} key={a.id}><div><b>{a.message}</b><small>{a.chain} · {new Date(a.createdAt).toLocaleString()}</small></div><button className="soft-action" onClick={()=>resolve(a.id)}>Resolve</button></div>)}</div>:<p style={{fontSize:11,color:"#8a8fa0"}}>No unresolved alerts. Watch a wallet below and alerts appear here automatically when it buys -- monitoring runs continuously in the background, no need to keep this page open.</p>}
 </section>;
}
