...
export default function TokenDetail({sel,opp,me,close,onTraded}:{sel:{chain:string;mint:string};opp:any;me:any;close:()=>void;onTraded:()=>void}){
 ...
 return <div className="token-detail">
  ...
  <section className="app-card"><div className="card-title"><div><span>TRACKED SMART MONEY</span><h2>Independent wallet evidence</h2></div><span className="status-badge">{data?.smartMoney?.summary?.distinctTrackedWallets||0} wallets</span></div>
   {data?.smartMoney?.relationships?.length?<><div className="review-grid">
    <div><span>MemeCloud Picks</span><b>{data.smartMoney.summary.memeCloudPicks}</b></div><div><span>Elite</span><b>{data.smartMoney.summary.elite}</b></div><div><span>Proven</span><b>{data.smartMoney.summary.proven}</b></div><div><span>Verified whales</span><b>{data.smartMoney.summary.whales}</b></div><div><span>Tracked buy flow</span><b>{money(data.smartMoney.summary.trackedBuyFlowUsd)}</b></div><div><span>Tracked net flow</span><b>{money(data.smartMoney.summary.netTrackedInflowUsd)}</b></div>
   </div><div className="list">{data.smartMoney.relationships.slice(0,20).map((r:any)=><div className="list-row" key={`${r.walletAddress}:${r.mint}`}><div><b>{r.label||`${r.walletAddress.slice(0,6)}…${r.walletAddress.slice(-4)}`} · {r.state.replaceAll("_"," ")}</b><small>{r.source} · {r.stage.replaceAll("_"," ")}{r.holdingVerification?` · ${r.holdingVerification.replaceAll("_"," ")}`:""}</small></div><span>{money(r.netFlowUsd)}</span><span>{timeAgo(r.latestActivityAt)}</span></div>)}</div></>:<div className="pnl-empty">No qualified tracked-wallet relationship has been observed for this token yet.</div>}
   <div className="notice" style={{marginTop:10}}>Holding labels describe the latest observed transaction balance only. MemeCloud does not claim a wallet still holds a token until a current balance is independently verified.</div>
  </section>
