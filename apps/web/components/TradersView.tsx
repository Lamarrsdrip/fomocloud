...
export default function TradersView({reload}:{platform:any[];follows:any[];followMap:Map<string,any>;setMode:(id:string,m:string)=>void;customOpen:boolean;setCustomOpen:(v:boolean)=>void;reload:()=>Promise<void>}){
 ...
 const[err,setErr]=useState(""),[open,setOpen]=useState<string|null>(null),[draft,setDraft]=useState<any>({});
 ...
 {!live.error&&<div className="trader-grid">{rows.map((t:any)=><article className="trader-card" key={t.id}>
  ...
  {open===t.id&&<div className="app-card" style={{marginTop:12,minWidth:0,overflow:"hidden"}}>
