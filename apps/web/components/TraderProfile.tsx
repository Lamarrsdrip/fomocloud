...
export default function TraderProfile({traderId,onBack,reload}:{traderId:string;onBack:()=>void;reload:()=>Promise<void>}){
 const[data,setData]=useState<any>(null),[err,setErr]=useState(""),[loading,setLoading]=useState(true),[tab,setTab]=useState("holdings"),[history,setHistory]=useState<any[]>([]),[nextBefore,setNextBefore]=useState<string|null>(null),[nextBeforeId,setNextBeforeId]=useState<string|null>(null),[moreLoading,setMoreLoading]=useState(false);
 async function load(){try{setErr("");const x=await apiFetch<any>(`/v1/curated/traders/${traderId}/profile?limit=100`);setData(x);setHistory(x.history||[]);setNextBefore(x.nextBefore||null);setNextBeforeId(x.nextBeforeId||null)}catch(e){setErr(plainError(e))}finally{setLoading(false)}}
 ...
 async function loadMore(){if(!nextBefore||moreLoading)return;setMoreLoading(true);try{const x=await apiFetch<any>(`/v1/curated/traders/${traderId}/history?limit=100&before=${encodeURIComponent(nextBefore)}${nextBeforeId?`&beforeId=${encodeURIComponent(nextBeforeId)}`:""}`);setHistory(h=>[...h,...(x.events||[])]);setNextBefore(x.nextBefore||null);setNextBeforeId(x.nextBeforeId||null)}catch(e){setErr(plainError(e))}finally{setMoreLoading(false)}}
