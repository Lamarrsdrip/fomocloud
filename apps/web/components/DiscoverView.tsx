...
export default function DiscoverView({brain,brainDegraded,setView,openToken}:{brain:any[];brainDegraded:boolean;setView:(v:any)=>void;openToken:(s:{chain:string;mint:string})=>void}){
 ...
 return <>
  {brainDegraded&&<div className="notice" style={{marginBottom:12,borderColor:"rgba(247,185,95,.3)"}}>Smart-money research is temporarily degraded — market data is rate-limited, so wallet-triggered opportunities are not being refreshed right now. This isn't "nothing happening," it's a real provider outage; existing evidence stays honest rather than looking falsely live.</div>}
  ...
  {rows.length?<div className="token-list hunt-list">{rows.map(o=><div className="token-row hunt-row" key={o.id} onClick={()=>openToken({chain:o.chain,mint:o.mint})}>
   ...
   </div>)}</div>:<Empty icon={TrendingUp} title={brainDegraded?"Discovery is temporarily paused":"Nothing here yet"} body={brainDegraded?"The market data provider is rate-limited right now, so MemeCloud isn't scoring new tokens. This will resume automatically once the provider recovers.":"MemeCloud is watching qualified wallets. Opportunities appear here only after tracked smart money buys and the Brain verifies the token."} action="Browse traders instead" onClick={()=>setView("traders")}/>}
 </>
}
