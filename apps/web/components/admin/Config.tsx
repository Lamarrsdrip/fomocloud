...
const CFG_LABELS:Record<string,string>={brain:"Global Brain",marketData:"Market data",execution:"Trade routing",signer:"Delegated signer",risk:"Risk defaults",fees:"Platform fee",email:"Email",push:"Push notifications",social:"X (social)",chains:"Chains",branding:"Branding"};
...
const SETTINGS_CATEGORIES=[
 ...
 {id:"networks",label:"Networks",blurb:"Where MemeCloud watches and trades.",keys:["chains","marketData"]},
 {id:"intelligence",label:"Market intelligence",blurb:"Global Brain and optional social context after a verified trader buy.",keys:["brain","social"]},
...
export function Config({d,reload,admin}:{d:any;reload:()=>void;admin:boolean}){
 ...
 const providerInfo:Record<string,{purpose:string;links:{label:string;url:string}[]}>={
  ...
  brain:{purpose:"Controls Global Brain thresholds after a verified Admin-trader swap creates an opportunity.",links:[]},
 ...
 const templates:any={
  brain:{autoEntryScore:76,notifyScore:65,snapshotMaxAgeMs:45000,solanaChainWideEnabled:false,solanaFlowConcurrency:12,profileTradeUsd:5000,bnbWs:"",ethWs:"",bnbUsd:0,ethUsd:0},
 ...
 social:{xBearerToken:"",xIntelligenceBearerToken:"",xMaxReadsPerHour:24,xMaxReadsPerDay:120,xOAuthClientId:"",xOAuthClientSecret:"",xOAuthCallbackUrl:""},
 ...
 const REQUIRED_SUMMARY:[string,string][]=[["marketData","Blockchain data"],["execution","Trade routing"],["push","Notifications"],["email","Email"]];
 ...
 return <div className="settings-shell">
  ...
  {screen==="detail"&&<section className="app-card">
   ...
   <div className="admin-form">
    {key==="brain"&&<><div className="form-grid"><Cfg label="Auto-entry score (1-100)" type="number" value={form.autoEntryScore} on={v=>field("autoEntryScore",Math.max(1,Math.min(100,Number(v))))}/><Cfg label="Notify score (1-100)" type="number" value={form.notifyScore} on={v=>field("notifyScore",Math.max(1,Math.min(100,Number(v))))}/><Cfg label="Max market snapshot age ms" type="number" value={form.snapshotMaxAgeMs} on={v=>field("snapshotMaxAgeMs",Math.max(5000,Number(v)))}/><Cfg label="Wallet-listener concurrency" type="number" value={form.solanaFlowConcurrency} on={v=>field("solanaFlowConcurrency",Math.max(2,Number(v)))}/></div><div className="notice">Wallet-first mode is enforced. Global Brain starts research only after an enabled Admin-tracked Solana wallet makes a verified economic swap. There is no random wallet or random-token discovery scanner.</div></>}
    ...
    {key==="social"&&<><SecretField label="X intelligence bearer token" {...secretProps("xIntelligenceBearerToken")}/><div className="form-grid"><Cfg label="X read hard limit / hour" type="number" value={form.xMaxReadsPerHour} on={v=>field("xMaxReadsPerHour",Math.max(0,Number(v)))}/><Cfg label="X read hard limit / day" type="number" value={form.xMaxReadsPerDay} on={v=>field("xMaxReadsPerDay",Math.max(0,Number(v)))}/></div><SecretField label="Legacy X bearer token (fallback only)" {...secretProps("xBearerToken")}/><Cfg label="X OAuth client ID" value={form.xOAuthClientId} on={v=>field("xOAuthClientId",v)}/><SecretField label="X OAuth client secret" {...secretProps("xOAuthClientSecret")}/><Cfg label="X OAuth callback URL" value={form.xOAuthCallbackUrl} placeholder="https://meme-api.xaucloud.io/auth/x/callback" on={v=>field("xOAuthCallbackUrl",v)}/><div className="notice">Optional late-stage context only. MemeCloud never searches X periodically and never uses posting credentials for research. Zero disables X intelligence reads; core wallet/market intelligence continues without it.</div></>}
    {key==="chains"&&<div className="chain-config"><div className="notice">Production wallet monitoring and execution are Solana-only. Other schema enum values are not live scanners and cannot be enabled from this screen.</div><label className="check-line"><input type="checkbox" checked disabled/><span>SOLANA</span><small>Admin-tracked wallet listener + verified swap execution</small></label></div>}
