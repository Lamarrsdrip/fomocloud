...
export function Whales({d,reload,admin}:{d:any;reload:()=>void;admin:boolean}){
 ...
 const[form,setForm]=useState<any>({chain:"SOLANA",address:"",label:"",curated:false,researchSource:"",researchReason:""});
 const[err,setErr]=useState("");
 ...
 async function add(e:React.FormEvent){e.preventDefault();setErr("");try{await apiFetch("/v1/admin/discovery/candidates",{method:"POST",body:JSON.stringify(form)});setOpen(false);setForm({chain:"SOLANA",address:"",label:"",curated:false,researchSource:"",researchReason:""});reload()}catch(e){setErr(plainError(e))}}
 return <>
  ...
  {open&&<section className="app-card" style={{marginBottom:10}}><form className="form-grid" onSubmit={add}><label className="field"><span>Chain</span><select value={form.chain} onChange={e=>setForm({...form,chain:e.target.value})}><option>SOLANA</option><option>BASE</option><option>ETHEREUM</option><option>BNB</option><option>ARBITRUM</option><option>AVALANCHE</option></select></label><label className="field span2"><span>Wallet address</span><input value={form.address} onChange={e=>setForm({...form,address:e.target.value})} required/></label><label className="field"><span>Label (e.g. trader name)</span><input value={form.label} onChange={e=>setForm({...form,label:e.target.value})}/></label><label className="field"><span>Classification</span><select value={form.curated?"CURATED":"PLATFORM"} onChange={e=>setForm({...form,curated:e.target.value==="CURATED"})}><option value="PLATFORM">Platform added</option><option value="CURATED">MemeCloud curated pick</option></select></label><label className="field"><span>Research source</span><input value={form.researchSource} onChange={e=>setForm({...form,researchSource:e.target.value})} placeholder="Public leaderboard / analyst" required={form.curated}/></label><label className="field span2"><span>Why this wallet was selected</span><textarea value={form.researchReason} onChange={e=>setForm({...form,researchReason:e.target.value})} required={form.curated}/></label>{err&&<div className="auth-error span2">{err}</div>}<button className="action-primary span2" style={{height:42,borderRadius:12}}>Watch &amp; analyze — scoring still decides trust</button></form></section>}
