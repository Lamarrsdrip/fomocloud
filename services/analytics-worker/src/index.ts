...
async function tick(){
 ...
 for(const u of users){try{
   ...
   const [cash,positions]=await Promise.all([
     ...
     db.position.findMany({where:{userId:u.id,mode:"LIVE"},select:{costUsd:true,entryTokenRaw:true,remainingTokenRaw:true,unrealizedPnlUsd:true,realizedPnlUsd:true,status:true}})
   ]);
   const available=cash.reduce((a,x)=>a+x.availableUsd,0),reserved=cash.reduce((a,x)=>a+x.inTradesUsd,0);
