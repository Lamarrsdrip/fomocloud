...
const config=await getConfig<any>("brain");
const chains=([
  {chain:"BNB",ws:config?.bnbWs||process.env.BNB_RPC_WS,nativeUsd:Number(config?.bnbUsd??0)},
  {chain:"ETHEREUM",ws:config?.ethWs||process.env.ETH_RPC_WS,nativeUsd:Number(config?.ethUsd??0)}
] as {chain:Chain;ws?:string;nativeUsd:number}[]).filter(x=>x.ws);
