# FomoCloud forensic trading-engine and go-live audit
...
5. Enforce each limit twice: FomoCloud decision layer and wallet/provider policy layer.
...
Minimum policy fields still required in FomoCloud’s schema/API:
...
`docs.fomo.com` documents an unrelated website social-proof/marketing-notification product (Events, Templates and marketing KPIs). It must not be connected to FomoCloud trader discovery.
...
2. Set strict provider policy and FomoCloud policy limits.
