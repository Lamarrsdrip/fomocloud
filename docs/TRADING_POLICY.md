...
# FomoCloud trading policy

## Plain-English goal

FomoCloud should behave like a disciplined meme-coin trader, not a blind copy button.

It watches verified source wallets. When they buy, FomoCloud asks:
...
liquidity, FomoCloud can hold the runner through +200%, +500%, +1000%, +5000% and beyond.
...
A meme token can wick violently, so FomoCloud should not use an unnecessarily tight static stop.
...
FomoCloud tightens/forces exit when several things deteriorate together:
...
FomoCloud must never claim it knows the exact bottom.
...
FomoCloud is designed for transaction/execution fees instead of subscription fees.
...
## What FomoCloud cannot truthfully promise
