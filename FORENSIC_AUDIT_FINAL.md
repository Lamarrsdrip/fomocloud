...
| M-49 | API monolith refactor | LARGE, IN PROGRESS | server.ts 2443→979 lines (60% reduction) via middleware.ts/providerHealth.ts/auth.ts/trading.ts (fixes #35-37) plus three real Express Router route-domain splits: authRoutes.ts (fix #58, 12 routes), walletRoutes.ts (fix #59, 7 routes, plus queues.ts), and adminRoutes.ts (fix #60, 30 routes -- the largest and most interconnected group) | Remaining in server.ts: /v1/me/* account/settings/dashboard/positions routes, manual-trade, traders, social, public brain/smart-wallets. Not LIVE VERIFIED against any real flow (no DATABASE_URL here) |
...
| C-26 | Monolith refactor | IN PROGRESS (M-50 DONE, M-49 remaining) | = M-49/50 |
...
| M-49/M-50/C-26 | API + frontend monolith refactor | LARGE (M-49 remaining) | M-49 in progress: server.ts (2443->2050 lines) now has middleware.ts + providerHealth.ts + auth.ts + trading.ts split out (fixes #35-37); route-domain splitting (mounting actual Express Routers for auth/wallets/admin/trading) deliberately not attempted without a live DB to verify route-mount wiring against. **M-50 is now fully DONE**: both apps/web/app/app/page.tsx (800->149 lines, fix #53) and apps/web/app/admin/page.tsx (654->93 lines, fix #54) are reduced to thin routing shells with every view/tab in its own component file. Only M-49's route-domain splitting remains open in this item |
