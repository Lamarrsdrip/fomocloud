...
| M-49 | API monolith refactor | IN PROGRESS | server.ts 2443→1832 lines via middleware.ts/providerHealth.ts/auth.ts/trading.ts (fixes #35-37) plus the first real Express Router route-domain split, authRoutes.ts (fix #58, 12 routes) | Admin/wallet/trading route groups not yet split into their own Routers; not LIVE VERIFIED against a real signup/login/refresh flow (no DATABASE_URL here) |
