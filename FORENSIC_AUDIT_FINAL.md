...
| M-35 | Add Funds — direct one-tap flow from every surface | PARTIAL | Home's Fund button now opens directly to the Receive/QR screen in one tap (fix #55) | Not literally every surface -- e.g. no low-balance nudge yet during manual buy (TokenDetail) when Trading Cash is insufficient |
...
| M-46 | Empty/degraded states differentiated | DONE | Fixes #27 (Home Pulse, SmartWalletsView), #56 (Admin Global Brain + Whales), #57 (Admin Tokens); CommunityView/ActivityView/TradersView reviewed and found to have no pipeline-freshness dependency to report (they reflect this account's own DB rows directly, not an aggregated/computed feed) -- a degraded badge there would be decorative, not evidence-based | Every genuinely pipeline-dependent surface identified this session now distinguishes degraded from quiet |
...
| M-49 | API monolith refactor | SUBSTANTIALLY DONE | server.ts 2443→979 lines (60% reduction) via middleware.ts/providerHealth.ts/auth.ts/trading.ts (fixes #35-37) plus three real Express Router route-domain splits: authRoutes.ts (12 routes), walletRoutes.ts (7 routes, plus queues.ts), and adminRoutes.ts (30 routes -- fixes #58-60). Remaining server.ts content (/v1/me/* account/settings/dashboard/positions, manual-trade, traders, social, public brain/smart-wallets) is a single cohesive "user account + public discovery" domain, not a second sprawling god-file -- 979 lines is a reasonable size for an Express entrypoint, judged not worth further splitting purely for line count | Further splitting (e.g. a userRoutes.ts) would still be mechanically straightforward if desired later; not LIVE VERIFIED against any real flow (no DATABASE_URL here) |
| M-50 | Frontend monolith refactor | DONE | app/page.tsx 800→149 lines and admin/page.tsx 654→93 lines, every view/tab now its own component under apps/web/components/ (fixes #42, #44-48, #53, #54) | The "shared-state design decision" concern raised earlier this session turned out unfounded -- every remaining component in both files took props only |
...
| C-20 | Empty/error states | DONE | = M-46 |
...
| C-26 | Monolith refactor | SUBSTANTIALLY DONE (M-50 fully done, M-49 substantially done) | = M-49/50 |
...
| 54 | M-50 COMPLETE for apps/web/app/admin/page.tsx: extracted all 14 admin tabs (Overview, BrainAdmin, UsersView, TradersAdmin, Signals, Trades, AdminPositions, FailedTrades, Tokens, Whales, Broadcasts, Audit, Health, and the largest/most interconnected one, Config, with its 6 module-level provider-status helpers) into apps/web/components/admin/*.tsx. File is now a 93-line shell (auth check/tab state/nav/routing only), down from 654 at session start -- an 86% reduction. M-50 is now fully complete for both god-files in the frontend | 36185dd..7c9406d (4 commits) | M-50 |
| 55 | Home's "Fund" button now opens directly to the Receive/QR screen in one tap (previously: navigated to Account, left the user to find and tap "Send / Receive / History" themselves to reach a screen that already existed and already defaulted to Receive) | 2b8c63e | M-35 (partial) |
| 56 | Admin Global Brain + Whales desks now surface pipelineDegraded/dataFreshnessSec (the exact same computation already done for the user-facing /v1/brain/feed and /v1/smart-wallets, reading the exact same tables, just never added to their admin equivalents) | ebef3f6 | M-46 (partial) |
| 57 | Admin Tokens desk now surfaces pipelineDegraded/dataFreshnessSec against discovery-worker's real lastSeenAt cadence, completing M-46 for every admin surface identified this session | ec4a343 | M-46 |
...
| M-33/42/43/47, C-13/16/17/23 | Remaining UX/product redesign — items not yet DONE per the M table above | LARGE | DONE already, not repeated here: Home Pulse (M-41), Token Detail Verdict (M-44), Auto Trade UX (M-45), status-language (C-21), empty/degraded states (M-46, fix #57), one-tap Fund from Home (M-35, fix #55, though not literally every surface). Genuinely still open: full unified Wallet layout (M-33), full Discover section redesign (M-42), Smart Money first-class nav (M-43), Admin Health 2.0 full dashboard with history/charts (M-47) |
| M-60/C-22 | Full mobile/viewport UX QA and design-system unification | LARGE, BLOCKED | No device/browser available in this environment to actually test against |
| M-49/M-50/C-26 | API + frontend monolith refactor | SUBSTANTIALLY DONE | **M-50 fully DONE**: both apps/web/app/app/page.tsx (800->149 lines, fix #53) and apps/web/app/admin/page.tsx (654->93 lines, fix #54) are thin routing shells with every view/tab in its own component file. **M-49 substantially done**: server.ts 2443->979 lines (60% reduction) via 4 helper-file extractions plus 3 real Express Router route-domain splits (auth/wallet/admin, fixes #58-60) -- remaining content is one cohesive user-account+public-discovery domain, not a second sprawling file |
...
## 2026-08-30 owner/degen continuation pass

A new source-level owner pass was completed from the uploaded full ZIP after live login recovery. See `OWNER_DEGEN_AUDIT_2026-08-30.md` for detailed traceability. This continuation specifically closes/strengthens the noisy Account/wallet/notification UX, one-balance presentation, Smart Money evidence/copy UX, skill-first wallet scoring, quality-weighted Brain convergence, early-repeat insider hints, qualified-only Hunt/New Token alerts, rug-evidence propagation, honest unknown token age, social/X priority, platform-watchlist global phone alerts, and Windows legacy-service port-conflict regression.

Status discipline remains unchanged: these source changes are **IMPLEMENTED** and the pure regression suites listed in that report are **TESTED** in the audit sandbox; full monorepo build and production behavior remain pending independent dependency-equipped build/deploy and observation/live verification.
