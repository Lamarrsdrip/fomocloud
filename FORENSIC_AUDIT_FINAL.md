...
| M-49 | API monolith refactor | IN PROGRESS | server.ts 2443→1553 lines via middleware.ts/providerHealth.ts/auth.ts/trading.ts (fixes #35-37) plus two real Express Router route-domain splits: authRoutes.ts (fix #58, 12 routes) and walletRoutes.ts (fix #59, 7 routes, plus queues.ts for the shared Redis/BullMQ instances) | Admin route group and the manual-trade/traders/discovery/social route groups not yet split into their own Routers; not LIVE VERIFIED against real auth/wallet-send flows (no DATABASE_URL here) |
