...
test("unknown token age is never treated as just-launched alpha", ()=>{
  assert.equal(dynamicChaseCapPct({...strong,ageMinutes:-1})<=dynamicChaseCapPct({...strong,ageMinutes:10}),true);
});
