...
test("missing metadata shows the real mint, never a fabricated name",()=>{
 const c=walletActivityContent(event);assert.match(c.title,/Dz9m…bonk/);assert.doesNotMatch(c.title,/Unknown token/);assert.equal(c.data.mint,mint);
});
test("metadata backfill immediately changes content without a new event",()=>{
 const c=walletActivityContent(event,{symbol:"USELESS",name:"USELESS COIN"});assert.equal(c.title,"🟢 @Rowdy bought USELESS");assert.ok(c.body.includes("$5K"));assert.equal(c.id,event.id);
});
